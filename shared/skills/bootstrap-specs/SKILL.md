...
For **Brownfield**, generate the same meta-files and progress tracker but no individual system specs. Also generate `PLAN.md` as the Bootstrap-owned **spec-extraction plan**, with system-to-code evidence mapping, discovery strategies, extraction focus, and authoring order. It directs an extracting agent to turn implementation evidence into fully authored prescriptive requirements, behavior rules, error handling, parameters, and acceptance/test scenarios; it is not a `create-plan` execution molecule, work-bead slice graph, or execution state. Code locations may appear in this extraction artifact, but not in the resulting system specs.
