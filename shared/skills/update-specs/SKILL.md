...
Read `AGENTS.md`, `specs/README.md`, the applicable `SPEC-OF-SPECS`, the relevant specs, and the normalized `git diff --stat` and complete `git diff`. Load [REFERENCE.md](REFERENCE.md) now and keep it available through classification, editing, review, and rollback for its evidence prompts, plan contract, and checklists.
...
- Load [REFERENCE.md](REFERENCE.md) after Git preflight and keep it loaded through classification, editing, all three reviews, and rollback because it contains the executable boundary recipe, evidence prompts, output forms, and detailed checklists.
