...
- Residual risks: GitHub API permissions, eventual consistency, host policy, and installed-client variation can still block a real invocation, so the skill requires runtime discovery and evidenced safe failure. Live PR creation/update and hosted CI mutation were deliberately not exercised. Pi visibility remains deferred to VG-001, tmux consolidation remains deferred to SK-033, and coordinator verification remains outstanding.
