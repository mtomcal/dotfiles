...
`MG-001` revision 2, `MG-002` revision 1, SK-001 through SK-011 are verified. SK-012 remains claimed. The next open lane may select an unblocked item disjoint from SK-012. No later production file is authorized without its item-specific exact proposal in `proposal-ready` state.
