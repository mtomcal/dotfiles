...
`MG-001` revision 2, `MG-002` revision 1, and SK-001 through SK-009 are verified. SK-010 and SK-011 are claimed as the active pair; SK-009 now supplies the verified terminology route consumed by SK-010, and the two active target/support scopes are disjoint. No later production file is authorized without its item-specific exact proposal in `proposal-ready` state.
