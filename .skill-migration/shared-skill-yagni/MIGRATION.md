...
| `SK-011` | skill | `prototype` | simplify inline | SK-001 | integrating |
...
`MG-001` revision 2, `MG-002` revision 1, and SK-001 through SK-009 are verified. SK-010 remains claimed and SK-011 is integrating; their target/support scopes are disjoint. No later production file is authorized without its item-specific exact proposal in `proposal-ready` state.
