...
| `SK-010` | skill | `update-specs` | retain substance; repair executable/ownership contract | SK-001, MG-001 | integrating |
...
`MG-001` revision 2, `MG-002` revision 1, SK-001 through SK-009, and SK-011 are verified. SK-010 is integrating and SK-012 remains claimed; their target/support scopes and ownership decisions are disjoint. No later production file is authorized without its item-specific exact proposal in `proposal-ready` state.
