...
<tool id="swarm">
    swarm CLI for worker management (spawn, ls, logs, kill, clean, wait)
</tool>

<tool id="beads">
    bd CLI for issue tracking (ready, show, update, close, sync)
</tool>
...
<workflow>
    <phase num="0" title="Validate Environment">
        ...
        <action>Run <tool id="cli" command="swarm ls" /> to check for existing workers</action>
        <action>Run <tool id="cli" command="git worktree list" /> to see existing worktrees</action>
        <decision>
    ...
    <phase num="1" title="Select Work" if="action=start">
        <action>Run <tool id="cli" command="bd ready" /> to show available work</action>
        <decision>
        ...
        <action>For each selected issue, run <tool id="cli" command="bd show {issue}" /> to understand requirements</action>
    ...
    <phase num="2" title="Spawn Workers" if="action=start">
        <action>Run <tool id="cli" command="swarm clean --all" /> to clear any stopped workers</action>
...
swarm spawn \
  ...
  "/beads-full-cycle id={issue-id} research={research} codeReviewPasses={codeReviewPasses} testReviewPasses={testReviewPasses} createNewBranch=true"
        ...
        <action>After all workers spawned, run <tool id="cli" command="swarm ls" /> to confirm</action>
        <action>Run <tool id="cli" command="git worktree list" /> to confirm worktrees created</action>
    ...
    <phase num="3" title="Monitor Workers" if="action=start OR action=monitor">
        <action>Run <tool id="cli" command="swarm ls" /> to check worker status</action>
        <action>For each running worker, run <tool id="cli" command="swarm logs {name} | tail -40" /> to check progress</action>
        <action>Check Beads for progress notes: <tool id="cli" command="bd show {issue-id}" /> for each issue</action>
...
| {name} | {id}  | {running/stopped} | {from logs} |
    ...
    <phase num="4" title="Rescue Incomplete Workers" if="action=rescue OR workers-incomplete">
        ...
        <steps>
            ...
            <step>If issue not closed: <tool id="cli" command="bd close {id} --reason '{summary}'" /></step>
            <step>If not pushed: <tool id="cli" command="git push -u origin HEAD" /></step>
            <step>If no PR: <tool id="cli" command="gh pr create --title '{title}' --body '{body}'" /></step>
        </steps>
        <action>Update Beads with PR URL: <tool id="cli" command="bd update {id} --notes 'Pull Request: {PR_URL}'" /></action>
        <action>Sync Beads: <tool id="cli" command="bd sync" /></action>
    </phase>

    <phase num="5" title="Verify Completion">
        <action>Run <tool id="cli" command="gh pr list --state open" /> to see all open PRs</action>
        <action>Run <tool id="cli" command="bd ready" /> to see remaining work</action>
...
| {id} | {priority} | {title} |
    ...
    <phase num="6" title="Cleanup" if="action=cleanup OR prs-merged">
        ...
        <command><tool id="cli" command="git worktree remove {worktree-path}" /></command>
        <action>Delete merged branches:</action>
        <command><tool id="cli" command="git branch -d {branch-name}" /></command>
        <action>Prune remote references:</action>
        <command><tool id="cli" command="git fetch --prune origin" /></command>
        <action>Clean stopped workers:</action>
        <command><tool id="cli" command="swarm clean --all" /></command>
        <action>Pull latest main:</action>
        <command><tool id="cli" command="git pull origin main" /></command>
        <action>Sync Beads:</action>
        <command><tool id="cli" command="bd sync" /></command>
    </phase>
</workflow>
