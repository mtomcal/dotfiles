...
<system-instructions>
    ...
    <purpose>Validate web application acceptance criteria from a Beads issue using Playwright-based browser automation with support for multiple concurrent browser contexts</purpose>
</system-instructions>
...
<workflow>
    ...
    <phase num="2" title="Get Beads Issue">
        <action>Ask the user for the Beads issue id (e.g. bd-a1b2) to test</action>
        <action>Read the full issue with <tool id="cli" command="bd show {id}" /></action>
        <action>Extract the acceptance criteria from the Beads issue</action>
...
# Acceptance Test Report: Beads {id}
...
- Viewport: {dimensions used}
    ...
    <phase num="7" title="Update Beads Issue and Sync">
        <action>Ask the user if they want to log the test results to the Beads issue</action>
        <choices>
            <choice id="Yes - log results" shortcut="y" />
            <choice id="No - skip logging" shortcut="n" />
        </choices>
        <action if="yes">Use <tool id="cli" command="bd update {id} --notes {summary of test results}" /> to log the test summary</action>
        <action if="yes">After updating, sync to git</action>
