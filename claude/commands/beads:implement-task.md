...
<workflow>
    <phase num="1" title="Initial Setup">
        ...
        <action>You must use the <tool id="cli" command="bd ready" /> to grab the next available issue</action>
        <action>You must read in full the selected issue with <tool id="cli" command="bd show {id}" /></action>
        <note>Beads uses hash-based IDs like bd-a1b2</note>
    </phase>
    <phase num="2" title="Ask user if reading last commit will help with the issue">
        ...
        <reason>Think deeply if the previous commit affects the implementation of the current issue in anyway</reason>
    </phase>
    ...
    <phase num="4" title="Implement the issue">
        <action>Implement the current Beads issue using test driven development</action>
    </phase>
    ...
    <phase num="8" title="Update issue and sync">
        ...
        <note>Beads uses --notes for progress tracking instead of --log</note>
    </phase>
</workflow>
